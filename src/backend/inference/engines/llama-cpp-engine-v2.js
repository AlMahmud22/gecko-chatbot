...
//// Llama.cpp Inference Engine - File-based Template System
//// Handles GGUF and GGML model formats
//// Uses node-llama-cpp library with file-based template loading
...
export class LlamaCppEngine extends BaseInferenceEngine {
  ...
  async initialize() {
    ...
    try {
      ...
      console.log('[llama.cpp] Engine initialized with file-based templates');
    } catch (err) {
  ...
  async loadModel(modelPath, config = {}) {
    ...
    try {
      ...
      // Detect model family using file-based template loader
      const templateFamily = llamaCppTemplateLoader.detectModelFamily(modelId);
      const template = llamaCppTemplateLoader.getTemplate(templateFamily);
  ...
  async generate(modelId, prompt, options = {}) {
    ...
    try {
      ...
      const response = await session.prompt(formattedPrompt, {
        ...
        stopStrings: options.stopStrings || stopTokens,
